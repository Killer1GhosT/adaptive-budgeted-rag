...
📄 **Paper:** [arXiv:2606.29090](https://arxiv.org/abs/2606.29090) (cs.CL, cross-listed cs.IR / cs.AI) · Elsevier journal version in submission
...
- **S3 — retrieval score variance.** Variance of the reranker scores over the
  ...
  (`experiments/07_signal_diagnostics.py`) showed the sign was backwards, and
  it is used as corrected.
...
```
experiments/            the full pipeline, numbered in run order
...
data/                   prepared evaluation datasets (HotpotQA, TriviaQA, sample)
logs/                   committed JSON/JSONL run outputs backing the paper's tables
paper/                  arXiv LaTeX source, bibliography, figures, compiled PDF
policy/                 reserved for future work — pluggable stopping policies
                        beyond the current threshold rule (empty)
```
...
Run in numeric order — each stage writes to `logs/` and later stages reuse
those files, so the expensive LLM stages run once:

```bash
python experiments/00_prepare_data.py
python experiments/00b_prepare_triviaqa.py
python experiments/01_retrieval.py  --dataset hotpotqa --n 500
python experiments/01b_open_retrieval.py --dataset hotpotqa --n 500
python experiments/02_rerank.py     --dataset hotpotqa --n 200
python experiments/03_generate_and_confidence.py --dataset hotpotqa --n 200 --backend local
python experiments/04_adaptive_loop.py --dataset hotpotqa --n 200 --backend local
python experiments/05_experiments.py   --dataset hotpotqa --n 200 --backend local
python experiments/06_ablations_and_sweep.py --dataset hotpotqa --backend local
python experiments/07_signal_diagnostics.py  --dataset hotpotqa --backend claude --n 200
python experiments/make_figures.py
```

Swap `--backend local` for `claude` or `ollama` to reproduce the other
backbones. The committed `logs/` files are the exact outputs behind the paper's
tables, so every number is checkable without re-running anything.
